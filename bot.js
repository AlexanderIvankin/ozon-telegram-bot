...
// Функция обновления активности админа
...
function startInactivityTimer() {
    ...
    inactivityInterval = setInterval(() => {
        if (scheduler.isCheckerPaused()) return; // если пауза – не трогаем
        if (!currentOrderProcessing) return; // нет активного заказа
        ...
        if (minutesSinceLastActivity >= AUTO_SKIP_MINUTES) {
            ...
            forceReloadQueue(); // функция, которая удаляет старое сообщение, сбрасывает текущий заказ и отправляет следующий
        }
